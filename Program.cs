...
namespace _6_task
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            Console.WriteLine("Choose number 1 to use Quicksort or number 2 to use Treesort");
            string choice = Console.ReadLine();
            switch (choice)
            {
                case "1":
                    Console.WriteLine("You chose Quicksort");
                    Console.WriteLine($"{processedString.QuickSorted()}");
                    break;
                case "2":
                    Console.WriteLine("You chose Treesort");
                    Console.WriteLine($"{processedString.TreeSorted()}");
                    break;
                default:
                    Console.WriteLine("You did not choose sort algoritm");
                    break;
            }
            int RandomInt;
            ...
            try
            {
                RandomInt= await randomservice.Next(processedString.Length - 1);
            }
            catch (HttpRequestException)
            {
                Console.WriteLine("Connection failed");
                RandomInt = new Random().Next(processedString.Length - 1);
            }
            var removedChar = processedString[RandomInt];
            Console.WriteLine($"{removedChar} was removed at index {RandomInt}");
            Console.WriteLine($"Old string: {processedString}");
            processedString = processedString.Remove(RandomInt, 1);
